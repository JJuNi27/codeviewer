  ...
  <div style="border:1px solid #333; border-radius:12px; padding:12px;">
    ...
    renderLoopTable(a) {
        ...
        const rows = loops
            ...
            const v = x.varName ?? "?";
            const it = x.iterExpr ?? "?";
            const role = idx === 0 ? "바깥(행 가능성↑)" : idx === 1 ? "안쪽(열 가능성↑)" : "내부";
            return `
<tr>
...
<table style="width:100%; border-collapse: collapse; font-size:13px;">
  ...
  <tbody>${rows}</tbody>
    ...
    renderFlowchart(a) {
        ...
        const boxes = loops
            .slice(0, 2)
            .map((x) => this.escapeHtml(x.raw ?? `for ${x.varName} in ${x.iterExpr}`));
...
<style>
  ...
  .cv-cursor{
    ...
    `;
        ...
        const animScript = (animRows && animCols)
            ? `
<script>
(() => {
  const R = ${animRows};
